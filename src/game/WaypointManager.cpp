...
void WaypointManager::Load()
{
    ...
    else
    {
        ...
        do
        {
            ...
            if (!cData)
            {
                sLog.outErrorDb("Table creature_movement contain path for creature guid %u, 但该 creature guid does not exist. 跳过.", id);
                continue;
            }
            ...
            if (be.emote)
            {
                if (!sEmotesStore.LookupEntry(be.emote))
                    sLog.outErrorDb("Waypoint path %u (Point %u) are using emote %u, 但 emote does not exist.",id, point, be.emote);
            }
    ...
    else
    {
        ...
        do
        {
            ...
            if (!cInfo)
            {
                sLog.outErrorDb("Table creature_movement_template references unknown creature template %u. 跳过.", entry);
                continue;
            }
            ...
            {
                if (!sEmotesStore.LookupEntry(be.emote))
                    sLog.outErrorDb("Waypoint template path %u (point %u) are using emote %u, 但 emote does not exist.", entry, point, be.emote);
            }
