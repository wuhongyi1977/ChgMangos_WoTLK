...
void PlayerMenu::SendGossipMenu(uint32 TitleTextId, ObjectGuid objectGuid)
{
    ...
    //DEBUG_LOG( "世界： Sent SMSG_GOSSIP_MESSAGE NPCGuid=%u",GUID_LOPART(npcGUID) );
}
...
void PlayerMenu::CloseGossip()
{
    ...
    //DEBUG_LOG( "世界： Sent SMSG_GOSSIP_COMPLETE" );
}
...
void PlayerMenu::SendPointOfInterest( float X, float Y, uint32 Icon, uint32 Flags, uint32 Data, char const * locName )
{
    ...
    GetMenuSession()->SendPacket( &data );
    //DEBUG_LOG("世界： Sent SMSG_GOSSIP_POI");
}
...
void PlayerMenu::SendTalking( uint32 textID )
{
    ...
    DEBUG_LOG(  "世界： Sent SMSG_NPC_TEXT_UPDATE " );
}
...
void PlayerMenu::SendQuestGiverQuestList(QEmote eEmote, const std::string& Title, ObjectGuid npcGUID)
{
    ...
    GetMenuSession()->SendPacket( &data );
    DEBUG_LOG("世界： Sent SMSG_QUESTGIVER_QUEST_LIST NPC Guid = %s", npcGUID.GetString().c_str());
}
...
void PlayerMenu::SendQuestGiverStatus( uint8 questStatus, ObjectGuid npcGUID )
{
    ...
    GetMenuSession()->SendPacket( &data );
    DEBUG_LOG( "世界： Sent SMSG_QUESTGIVER_STATUS for %s", npcGUID.GetString().c_str());
}
...
void PlayerMenu::SendQuestGiverQuestDetails(Quest const *pQuest, ObjectGuid npcGUID, bool ActivateAccept)
{
    ...
    DEBUG_LOG("世界： Sent SMSG_QUESTGIVER_QUEST_DETAILS NPCGuid = %s, questid = %u", npcGUID.GetString().c_str(), pQuest->GetQuestId());
}
...
void PlayerMenu::SendQuestQueryResponse( Quest const *pQuest )
{
    ...
    DEBUG_LOG( "世界： Sent SMSG_QUEST_QUERY_RESPONSE questid=%u", pQuest->GetQuestId() );
}
...
void PlayerMenu::SendQuestGiverOfferReward(Quest const* pQuest, ObjectGuid npcGUID, bool EnableNext)
{
    ...
    GetMenuSession()->SendPacket(&data);
    DEBUG_LOG("世界： Sent SMSG_QUESTGIVER_OFFER_REWARD NPCGuid = %s, questid = %u", npcGUID.GetString().c_str(), pQuest->GetQuestId());
}

void PlayerMenu::SendQuestGiverRequestItems(Quest const *pQuest, ObjectGuid npcGUID, bool Completable, bool CloseOnCancel)
{
    // We can always call to RequestItems, 但该 packet only goes out if there are actually
    ...
    GetMenuSession()->SendPacket( &data );
    DEBUG_LOG("世界： Sent SMSG_QUESTGIVER_REQUEST_ITEMS NPCGuid = %s, questid = %u", npcGUID.GetString().c_str(), pQuest->GetQuestId());
}
