...
void RealmList::UpdateRealms(bool init)
{
    ...
    if(result)
    {
        do
        {
            ...
            {
                sLog.outError("服务器标识设置错误！允许类型：离线(Mask 0x2), 新角色(mask 0x20), 测试(mask 0x40), 开发(mask 0x04)");
            ...
            if(init)
                sLog.outString("添加服务器 \"%s\"", sLog.Utf8ToUnicode(fields[1].GetString()));
